...
**适用**：EPUB、PDF 本地 Kindle/电子阅读器书籍

**格式覆盖说明**：
- **EPUB**：完整支持，见下方脚本
- **PDF**：完整支持（文字型 + 扫描版），见下方脚本
- **MOBI / AZW3**（Kindle 格式）：不直接支持。处理方式：用 [Calibre](https://calibre-ebook.com) 将 MOBI 转换为 EPUB 后再处理（`ebook-convert input.mobi output.epub`），或用 [KindleUnpack](https://github.com/kevinhendricks/KindleUnpack) 解包。
...
def extract_epub(epub_path: str):
    ...
    print(f"[EPUB] {title}，共 {len(chapters)} 章")
...
**适用**：微信公众号、今日头条、知乎、Medium、Substack、少数派、掘金、各类博客单篇……任何「刷到一篇好文章想存进知识库」的场景。

**和 1.1 的区别**：1.1 是系统性消化整个网站，1.7 是轻量随手收藏——一行命令，一篇文章，进入 raw/，后续自动纳入 Wiki。

**核心工具**：Jina Reader（`r.jina.ai`，无需安装，覆盖大多数平台）是首选；遇到强反爬平台（微信、头条）再用专项方案。
...
|------|---------|-------|------|
| 知乎 | Jina Reader | ★★★★★ | 直接可用 |
| 少数派 | Jina Reader | ★★★★★ | 直接可用 |
| Medium | Jina Reader | ★★★★☆ | 付费文章会截断 |
| Substack | Jina Reader | ★★★★★ | 直接可用 |
| 掘金 | Jina Reader | ★★★★★ | 直接可用 |
| 今日头条 | Jina Reader → Playwright 降级 | ★★★☆☆ | 经常触发反爬，需降级 |
| 微信公众号 | 专项方案（见下） | ★★★☆☆ | 强反爬，不能直接爬 |
| 其他博客 | Jina Reader | ★★★★☆ | 绝大多数可用 |

---

### 通用提取脚本 `extract_article.py`

```python
# extract_article.py — 单篇文章提取，自动识别平台选择最优方案
...
if os.path.exists(PROCESSED_LOG):
    _processed.update(l.strip() for l in open(PROCESSED_LOG) if l.strip())
...
# ── 方案 A：Jina Reader（通用首选）──────────────────────────────────

def fetch_jina(url: str) -> str | None:
    """直接 curl Jina Reader，返回 Markdown 文本"""
    r = subprocess.run(
        ["curl", "-s", "--max-time", "30",
    ...
    if r.returncode != 0 or len(r.stdout.strip()) < 100:
        return None
    # Jina 返回内容过短（< 100 字符）通常说明被反爬拦截
    return r.stdout.strip()


# ── 方案 B：Playwright 降级（Jina 失败时）──────────────────────────

def fetch_playwright(url: str) -> str | None:
    """用 headless 浏览器渲染后提取正文"""
    try:
        from playwright.sync_api import sync_playwright
        import markdownify
    except ImportError:
        print("  [提示] 安装 playwright 和 markdownify 以启用降级方案：")
        print("         pip install playwright markdownify && playwright install chromium")
        return None
...
def fetch_wechat(url: str) -> str | None:
    """
    微信公众号文章反爬最强，有以下几种处理路径（按可行性排序）：

    推荐路径1：用手机「复制链接」后通过 Jina 提取（有时可用）
    推荐路径2：用「微信读书」或「微信文章助手」类 Chrome 插件手动导出 Markdown
    推荐路径3：在微信 PC 客户端打开文章，复制全文粘贴到剪贴板
    推荐路径4：用 Playwright + Cookie（需要先登录微信网页版，有失效风险）
    """
    # 先试 Jina（部分公众号文章可用）
    ...
    # Jina 失败，提示用户手动处理
    print(f"""
  [微信公众号] 自动提取失败，请使用以下任一手动方案：
  
  方案A（最简单）：在浏览器打开文章 → 全选复制 → 粘贴到文本文件
    保存到: ./raw/article__{url_to_slug(url)}.md
  
  方案B（格式更好）：安装 Chrome 插件 "Copy as Markdown" 或 "MarkDownload"
    打开文章 → 点击插件 → 一键复制 Markdown → 粘贴保存
  
  方案C：微信 PC 客户端打开 → 右键"复制文章到剪贴板"
...
def fetch_toutiao(url: str) -> str | None:
    """今日头条有 JS 渲染 + 频率限制，优先 Jina，失败降 Playwright"""
    content = fetch_jina(url)
    if content and len(content) > 300:
        return content
    print("  [今日头条] Jina 失败，尝试 Playwright...")
    return fetch_playwright(url)


# ── 主函数 ────────────────────────────────────────────────────────

def extract_article(url: str, title: str = "", force: bool = False) -> Path | None:
    """
    提取单篇文章到 ./raw/article__xxx.md

    url:   文章 URL
    title: 可选，手动指定标题（自动提取失败时使用）
    force: True 则忽略增量登记，重新提取
    """
    if not force and url in _processed:
        print(f"[跳过] 已处理: {url}")
        return None

    platform = detect_platform(url)
    print(f"提取 [{platform}]: {url}")

    content = None
    if platform == "wechat":
        content = fetch_wechat(url)
    elif platform == "toutiao":
        content = fetch_toutiao(url)
    else:
        content = fetch_jina(url)
        if not content or len(content) < 200:
            print("  [Jina 失败] 尝试 Playwright 降级...")
    ...
    # 从内容中尝试提取标题（Jina 通常在第一行输出 # 标题）
    if not title:
        first_line = content.split("\n")[0].lstrip("#").strip()
        title = first_line[:80] if first_line else url

    slug = url_to_slug(url)
    fname = f"article__{slug}.md"
...
def extract_articles_batch(urls: list[str]):
    """批量提取多篇文章"""
    success, failed = 0, 0
    for url in urls:
        result = extract_article(url.strip())
        if result:
            success += 1
        else:
            failed += 1
...
# 使用示例：
# extract_article("https://zhuanlan.zhihu.com/p/xxxxxxxxx")
# extract_article("https://mp.weixin.qq.com/s/xxxxxxxxx")
# extract_articles_batch(open("article_urls.txt").readlines())
```
...
python -c "
from extract_article import extract_articles_batch
extract_articles_batch(open('article_urls.txt').readlines())
"
...
python -c "
from extract_article import extract_article
extract_article('https://mp.weixin.qq.com/s/xxxxxxxx')
"
```
...
对于反爬平台，用浏览器插件是最省力的方案，提取结果直接存入 `./raw/`：
...
| [Joplin Web Clipper](https://joplinapp.org/clipper/) | Chrome / Firefox | 保存到本地 Joplin，可导出 Markdown |
...
微信是最难的平台，按可行性排序的完整选项：

```
优先级1：Jina Reader 直接提取（部分文章可用，约 40% 成功率）
    ↓ 失败
优先级2：MarkDownload 插件（在浏览器打开文章时可用）
    ↓ 无法用浏览器打开
优先级3：微信 PC 客户端「复制文章链接」→ 浏览器打开 → MarkDownload
    ↓ 仍然不行
优先级4：全选正文 → 复制 → 粘贴到 VS Code → 手动保存为 .md
    ↓ 量大时
优先级5：使用第三方转存服务（如「公众号下载器」类工具，效果不稳定）
```
...
当工程有 10 万行以上代码时，全量提取不现实。分优先级处理：
...
# 第1层：架构文档（最高价值）
docs = list(Path(PROJECT).rglob("README*")) + \
       list(Path(PROJECT).rglob("ARCHITECTURE*")) + \
       list(Path(PROJECT).rglob("docs/**/*.md"))

# 第2层：接口定义（类型/接口文件）
interfaces = list(Path(PROJECT).rglob("*.d.ts")) + \
             list(Path(PROJECT).rglob("**/types.py")) + \
             list(Path(PROJECT).rglob("**/interfaces.py"))

# 第3层：测试文件（反映真实使用方式）
tests = list(Path(PROJECT).rglob("**/*.test.*"))[:50]

# 第4层：入口文件
entries = list(Path(PROJECT).rglob("index.*"))[:20] + \
...
priority_files = docs + interfaces + tests + entries + recent_files
print(f"优先处理 {len(priority_files)} 个文件")
```
...
| 单篇文章（通用平台） | 1 篇 | 3-5 秒 | — | — | 无 |
| 单篇文章（微信公众号） | 1 篇 | 1-3 分钟（含手动操作） | — | — | 无 |
...
| 单篇文章 | Jina Reader | 直接 curl | 无需安装，通用平台首选 |
| 单篇文章（降级） | playwright | `pip install playwright markdownify` | 反爬平台（头条等）降级 |
...
| 2026-06-01 | 删除 YouTube 相关全部内容（中国大陆网络不可访问）：移除 1.2 YouTube 子节及脚本、yt__ 前缀、youtube-transcript-api 工具、成本表 YouTube 行、Quick Start 场景 B 改为 Bilibili |
