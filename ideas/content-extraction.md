...
| 2026-06-06 | 新增 2.1B 困惑驱动的第二轮提取（陌生工程专用）：让 Claude 先读一遍再列盲区问题，用问题驱动定向 grep/glob，比「提取所有东西」更精准；新增 2.1C Runtime 维度补充（日志样本/错误类型分布/API样本/schema）；Phase 1A 扩展 Git 热力图提取（高频变更文件 Top 20）；新增 Phase 1C 关键文件完整变更历史提取脚本；3.6 节重排信息密度优先级（测试升为第1层，并说明原因） |
